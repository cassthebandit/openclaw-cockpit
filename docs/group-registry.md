...
| --- | --- | --- | --- | --- |
| 0 | `Active Agents` | Live or resumable agent TUIs: starting, running, waiting, blocked, review, approval/prompt, or real resumed work. Live held agents stay here with a hold indicator. | tmux liveness, `agent_wall.py`, semantic lifecycle | Open by default; auto-open only on edge transition into active/urgent state. |
| 1 | `Held / Teardown Blocked` | Completed, delivered-idle, stale, or dead agent panes blocked by explicit hold or held+marked conflict. | `@oc_hold_reason`, janitor refusal | Open when non-empty; auto-open on new hold/blocker edge. |
| 2 | `Marked For Teardown` | Sessions actually marked by the janitor and eligible for countdown because no hold/evidence conflict blocks the countdown. | `@oc_teardown_marked_at`, `@oc_janitor_state`, janitor status sidecar | Open when non-empty; auto-open on new countdown edge. |
| 3 | `Cleanup Blocked` | Non-active cleanup debt that cannot safely be marked or killed because required contract data is missing or invalid. | janitor refusal reason, evidence/run-root validation | Open when non-empty; respect manual collapse after the new-blocker edge is acknowledged. |
| 4 | `Failed Agents` | Failed or terminal-problem agent panes inside the configured failure-visible window, or failed panes still awaiting operator judgment. | `@oc_state`, pane exit status, semantic lifecycle | Open on new failure edge; respect manual collapse afterward. |
| 5 | `Operational Failures` | OpenClaw workflow/runtime delivery failures that need operator judgment. | runtime card presentation group | Respect manual collapse after initial/new failure edge. |
| 6 | `Sub-System Failures` | Platform, route, skeleton, source-health, or subsystem failures. | runtime card presentation group | Respect manual collapse after initial/new failure edge. |
...
An auto-open event is a transition, not steady-state presence. A group may auto-open only when a session enters that group's urgent/opening condition since the last snapshot or since the operator manually acknowledged/collapsed it.
