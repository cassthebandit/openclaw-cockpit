...
Only visible, expanded, detached, single-window/single-pane managed agent sessions qualify. Attached, linked, split, dead, display-only and copy-mode panes are not resized. Server-side checks revalidate session/window/pane/PID and ownership before mutations. A window-local owner token prevents competing Cockpits from resizing each other's targets. The next structural poll reconciles changes; fitting is single-flight and never runs from View. Growing and shrinking use the same rendered geometry, with no source-height cap feeding back into allocation.
